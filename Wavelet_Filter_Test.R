...
data <- read_delim("https://raw.githubusercontent.com/cefet-rj-dal/tspredit/main/develop/Etanol_df.csv",
                   delim = ";",
                   escape_double = FALSE,
...
filter <- ts_fil_wavelet_bkp(filter = c("haar", "d4", "la8", "bl14", "c6"))
filter <- fit(filter, ts$series)
y <- transform(filter, ts$series)
plot(plot_ts_pred(y=ts$series, yadj=y))
